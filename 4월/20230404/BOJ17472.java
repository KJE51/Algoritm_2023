...
public class BOJ17472 {
	...
	static class Bridge {
		...
		public Bridge(int island1, int island2,int size) {
			super();
			if (island1 < island2) {
...
//		System.out.println(Arrays.toString(arr[i]));
			...
			for (int k = 1; k < m; k++) {
				...
				if (arr[i][k] != arr[i][k - 1] && arr[i][k] == 0) {
					...
					if (len == 1 || k == m || arr[i][k] == startIsland) {
						...
						bridges.add(new Bridge(startIsland, arr[i][k], len));
				}
			}
		}
		...
		// dfs를 통한 부분집합 돌리기, 파라미터는 현재 bridges의 인덱스, 다리 길이
	...
	private static void dfs(int idx, int len) {
		...
		if (union(island1, island2)) {
			visited[idx] = true;
			dfs(idx + 1, len + bridges.get(idx).size);
			visited[idx] = false;
			for (int i = 0; i < islandN + 1; i++) {
